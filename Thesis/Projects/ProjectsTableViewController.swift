...
class ProjectsTableViewController: UITableViewController {
	...
	func getProjectsFromDatabase() {
		...
		projectsRef.observeSingleEvent(of: .value) { (snapshot) in
			for snap in snapshot.children {
				...
				let urlString = projectDict["imageURL"] as! String
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		DispatchQueue.main.async {
			// Configure the cell...
			cell.textLabel?.text = self.projectsArray[indexPath.row].title
			cell.detailTextLabel?.text = self.projectsArray[indexPath.row].address
			
			let url = URL(string: self.projectsArray[indexPath.row].image!)
			let resource = ImageResource(downloadURL: url!)
			cell.imageView?.kf.setImage(with: resource, completionHandler: { (result) in
				switch result {
				case .success(_):
					print("Successfully gathered photos from firebase storage")
